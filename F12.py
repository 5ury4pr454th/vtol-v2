...
from st_fbi_control import lateralController, altitudeController
...
t_rise_alt = 4.5
t_rise_lat = 4.5
# best time is 4.5 with integrator pole at -0.3

p_i = np.array([-0.3])
K_lat, ki_lat = ctrlb.integral_gains_from_time(t_rise_lat, p_i, alt = False)
K_alt, ki_alt = ctrlb.integral_gains_from_time(t_rise_alt, p_i)
print(f"ki values:{ki_lat} {ki_alt}")
...
alt_control = altitudeController(K_alt, ki_alt, p.t_step)
lat_control = lateralController(K_lat, ki_lat, p.t_step)
